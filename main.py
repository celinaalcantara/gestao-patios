...
def persist_data(data_string):
    try:
        ...
        if vision_data:
            ...
            if vision_status == 'ocupada':
                ...
                if patio_api_client.register_moto(moto_payload):
                    final_status = 'OK'
                    print(f"SUCESSO: Leitura em '{location}' validada pela visão e registrada.")
                else:
                    final_status = 'ERRO_REGISTRO'
            elif vision_status == 'livre':
                final_status = 'INCONSISTENTE'
                print(f"ALERTA: Inconsistência detectada em '{location}'. Sensor viu moto, mas visão reportou 'livre'.")
            else:
        ...
        columns_order = ['moto_id', 'device_id', 'location', 'timestamp', 'status']
        
        if os.path.exists(data_file):
            # Usar keep_default_na=False para evitar que 'NA' seja lido como NaN
            df = pd.read_csv(data_file, keep_default_na=False, na_values=[''])
            df = pd.concat([df, new_data], ignore_index=True)
        else:
            df = new_data
        
        df[columns_order].to_csv(data_file, index=False)
...
def mock_vision_api(location_id):
    ...
    mock_status = random.choices(['ocupada', 'livre'], weights=[0.8, 0.2], k=1)[0]
...
# patio_api_client = patio_vision_api("http://localhost:8080") # Para usar a API Java
patio_api_client = patio_vision_api("http://127.0.0.1:5000/mock") # Para usar o mock embutido
